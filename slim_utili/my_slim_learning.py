#copied from /usr/lib/python3.5/site-packages/tensorflow/contrib/slim/python/slim/learning.py
...
def train_step(sess, train_op, global_step, train_step_kwargs):
  ...
  if 'train_feed_dict' in train_step_kwargs:
      train_feed_dict = train_step_kwargs['train_feed_dict']
      assert type(train_feed_dict) in (dict, OrderedDict)
      train_feed_dict = dict(train_feed_dict)
  else:
  ...
  if 'should_trace' in train_step_kwargs:
    ...
    if sess.run(train_step_kwargs['should_trace'], feed_dict=train_feed_dict):
  ...
  if 'should_log' in train_step_kwargs:
    if sess.run(train_step_kwargs['should_log'], feed_dict=train_feed_dict):
  ...
  if 'should_stop' in train_step_kwargs:
    should_stop = sess.run(train_step_kwargs['should_stop'], feed_dict=train_feed_dict)
  else:
    should_stop = False
...
def train(train_op,
  ...
  #Chong added
  if train_feed_dict is not None:
    train_step_kwargs['train_feed_dict'] = train_feed_dict
  #Chong added end
    ...
    try:
      ...
      with sv.managed_session(
        ...
        if is_chief and sync_optimizer is not None:
          ...
          sess.run(init_tokens_op, feed_dict=train_feed_dict)
        try:

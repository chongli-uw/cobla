...
def get_channel_index(data_format=DATA_FORMAT_NHWC):
...
@tf.contrib.framework.add_arg_scope
def get_channel_dim(shape, data_format=DATA_FORMAT_NHWC):
  assert data_format != INVALID
  assert len(shape) == 4
  if data_format == 'NHWC':
    return int(shape[3])
...
@tf.contrib.framework.add_arg_scope
def global_avg_pool(x, data_format=DATA_FORMAT_NHWC):
...
@tf.contrib.framework.add_arg_scope
def factorized_reduction(net, output_filters, stride, data_format=DATA_FORMAT_NHWC):
